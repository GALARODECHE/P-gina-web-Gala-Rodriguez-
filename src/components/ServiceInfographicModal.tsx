...
interface ServiceInfographicModalProps {
  ...
  onRequestInfo?: (serviceTitle?: string) => void;
}
...
📲 WhatsApp / Tel: ${profile.phone || '+34 697 166 126'}`;
  ...
  const handleActionClick = (serviceTitle: string) => {
    if (onRequestInfo) {
      onRequestInfo(serviceTitle);
    } else {
      const cleanPhone = profile.whatsappNumber.replace(/[^0-9]/g, '');
      const text = encodeURIComponent(`Hola Gala, me gustaría solicitar información sobre el servicio: ${serviceTitle}`);
  ...
  // Helper function to return a distinctive icon for talks
        ...
        <div className="overflow-y-auto p-6 sm:p-10 space-y-8 print:p-0 print:overflow-visible print:text-black">
                ...
                <div className="p-6 rounded-3xl bg-[#f4f9fd] dark:bg-slate-800/80 border border-sky-200 dark:border-slate-700 flex flex-col justify-between space-y-4 print:border-stone-400 hover:border-teal-400 transition-all shadow-2xs">
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2 text-xs text-slate-600 dark:text-slate-300 pt-3 border-t border-sky-200 dark:border-slate-700">
                      ...
                      <li className="flex items-start gap-2">
                        <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                        <span>Anamnesis clínica integral directa y evaluación</span>
                      </li>
                      <li className="flex items-start gap-2">
                        <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                      ...
                      <li className="flex items-start gap-2">
                        <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                        <span>Resolución de dudas por email durante 15 días</span>
                      </li>
                    </ul>
                  </div>
                ...
                <div className="p-6 rounded-3xl bg-[#f4f9fd] dark:bg-slate-800/80 border border-sky-200 dark:border-slate-700 flex flex-col justify-between space-y-4 print:border-stone-400 hover:border-teal-400 transition-all shadow-2xs">
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2 text-xs text-slate-600 dark:text-slate-300 pt-3 border-t border-sky-200 dark:border-slate-700">
                      ...
                      <li className="flex items-start gap-2">
                        <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                        <span>Adaptación a viajes, eventos o cambios de rutina</span>
                      </li>
                    </ul>
                  </div>
          ...
          {(activeTab === 'all' || activeTab === 'talks') && (
            <div className="space-y-4 pt-2">
              ...
              {/* 4 Detailed Talk Cards with distinctive icons */}
