...
Fecha de solicitud: ${new Date().toLocaleString('es-ES')}`;
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-xl max-h-[90vh] bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-150"
        role="dialog"
        aria-modal="true"
        aria-labelledby="modal-title"
      >
        {/* Fixed Sticky Header - ALWAYS VISIBLE AT THE TOP */}
        <div className="sticky top-0 z-30 px-5 py-4 sm:px-6 sm:py-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-2xl bg-teal-100 dark:bg-teal-950/80 text-teal-800 dark:text-teal-300 flex items-center justify-center shrink-0 border border-teal-200 dark:border-teal-800 shadow-xs">
              <Calendar className="w-5 h-5" />
            </div>
            <div className="min-w-0">
              <h3
                id="modal-title"
                className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white leading-tight truncate"
              >
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 sm:p-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 dark:hover:text-white border border-slate-200 dark:border-slate-700 transition-all cursor-pointer shrink-0 shadow-xs flex items-center justify-center group"
        ...
        <div className="overflow-y-auto p-5 sm:p-6 space-y-5 flex-1 overscroll-contain">
          {isSubmitted ? (
            <div className="text-center py-2 space-y-5 animate-in zoom-in-95">
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                <button
                  type="button"
                  onClick={onClose}
                  className="w-full py-3 px-5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-200 transition-colors cursor-pointer flex items-center justify-center gap-2"
                >
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div className="p-3 rounded-2xl bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800/60 flex items-center gap-2.5 text-xs text-teal-900 dark:text-teal-200">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs sm:text-sm font-semibold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
                >
                  {services.map((s) => (
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-2">
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  <button
                    type="button"
                    onClick={onClose}
                    className="w-full sm:w-auto px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 font-bold text-xs sm:text-sm text-slate-700 dark:text-slate-300 transition-colors cursor-pointer text-center order-2 sm:order-1"
                  >
                    Volver a la Página
                  </button>
                  <button
                    type="submit"
                    className={`w-full sm:flex-1 py-3.5 px-4 rounded-xl font-bold text-xs sm:text-sm shadow-lg transition-all flex items-center justify-center gap-2 active:scale-98 cursor-pointer order-1 sm:order-2 ${theme.primary}`}
                  >
